
ARG UBUNTU_VERSION=22.04
ARG REMOTE=astrobee
FROM ${REMOTE}/astrobee:latest-humble_base-ubuntu${UBUNTU_VERSION}

# Install gazebo classic (11) and GTSAM from ppa
# Note: This is the last release of classic. No longer available in noble
RUN apt-get update && apt-get install -y software-properties-common \
    && add-apt-repository ppa:openrobotics/gazebo11-gz-cli \
    && apt update


RUN apt-get update && apt-get install -q -y --fix-missing \
    gazebo11 \
    ros-humble-desktop \
    ros-humble-gtsam \
    binutils \
    mesa-utils \
    x-window-system \
    ros-humble-gazebo-ros-pkgs \
    libgoogle-glog-dev libgflags-dev libgtest-dev \
    libluajit-5.1-dev \
    ros-humble-xacro \
    ros-humble-ros-testing \
    && rm -rf /var/lib/apt/lists/*

# Install Astrobee----------------------------------------------------------------
COPY ./scripts/setup/debians /setup/astrobee/debians

RUN apt-get update \
  && /bin/bash /setup/astrobee/debians/build_install_debians.sh \
  && rm -rf /var/lib/apt/lists/* \
  && rm -rf /setup/astrobee/debians

# COPY ./scripts/setup/packages_*.lst /setup/astrobee/
# note apt-get update is run within the following shell script
# RUN /setup/astrobee/install_desktop_packages.sh \
#   && rm -rf /var/lib/apt/lists/*
